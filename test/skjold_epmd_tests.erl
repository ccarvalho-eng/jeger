...
check_epmd_localhost_test() ->
    ...
    Result = skjold_epmd:check_epmd("localhost", 1000),
...
check_epmd_invalid_host_test() ->
    ...
    Result = skjold_epmd:check_epmd("192.168.255.255", 100),
    ?assertMatch({error, _}, Result).

query_names_invalid_host_test() ->
    %% Test querying names on invalid host
    Result = skjold_epmd:query_names("192.168.255.255", 100),
    ?assertMatch({error, _}, Result).

query_port_invalid_host_test() ->
    %% Test querying port on invalid host
    Result = skjold_epmd:query_port("192.168.255.255", "testnode", 100),
    ?assertMatch({error, _}, Result).
